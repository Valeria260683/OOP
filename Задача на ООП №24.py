"""Необходимо создать приложение для учета задач, которое будет
использовать классы Task, TaskList и TaskManager.
...
метод mark_done, который принимает задачу (экземпляр Task) и устанавливает ей
истинный статус выполнения
метод mark_undone, который принимает задачу (экземпляр Task) и устанавливает ей
ложный статус выполнения
...
class Task:
    ...
    def display(self):
        status = "Сделана" if self.status else "Не сделана"
